...
/**
 * Extracts UML-style relations between classes/interfaces from a parsed Java
 * AST.
 * Given a CompilationUnit and a set of allowed simple class names, this class
 * analyzes class/interface declarations to infer relationships for UML
 * diagrams.
 *
...
public final class JavaRelationExtractor {
    ...
    public static List<Relation> giveRelations(CompilationUnit cu, Set<String> allowed) {
        ...
        for (ClassOrInterfaceDeclaration classDecl : classDecls) {
            ...
            classDecl.getExtendedTypes()
                    .forEach(t -> addRelations(relations, allowed, src, simple(t), Relation.Kind.INHERITANCE));

            classDecl.getImplementedTypes()
                    .forEach(t -> addRelations(relations, allowed, src, simple(t), Relation.Kind.IMPLEMENTATION));
            ...
            for (MethodDeclaration methodDecl : classDecl.getMethods()) {
                methodDecl.getParameters().forEach(p -> referencedTypeNames(p.getType())
                        .forEach(name -> addRelations(relations, allowed, src, name, Relation.Kind.DEPENDENCY)));
                methodDecl.getBody().ifPresent(body -> {
                    body.findAll(VariableDeclarationExpr.class)
                            .forEach(localVar -> referencedTypeNames(localVar.getElementType()).forEach(
                                    name -> addRelations(relations, allowed, src, name, Relation.Kind.ASSOCIATION)));
                });
            }

            for (ConstructorDeclaration constructorDecl : classDecl.getConstructors()) {
                constructorDecl.getParameters().forEach(p -> referencedTypeNames(p.getType())
                        .forEach(dst -> addRelations(relations, allowed, src, dst, Relation.Kind.DEPENDENCY)));
                constructorDecl.getBody().findAll(VariableDeclarationExpr.class)
                        .forEach(localVar -> referencedTypeNames(localVar.getElementType())
                                .forEach(dst -> addRelations(relations, allowed, src, dst, Relation.Kind.ASSOCIATION)));
                constructorDecl.getBody().findAll(ObjectCreationExpr.class)
                        .forEach(newExpr -> referencedTypeNames(newExpr.getType())
                                .forEach(dst -> addRelations(relations, allowed, src, dst, Relation.Kind.DEPENDENCY)));
            }
        }
        return relations;
    ...
    private static void addRelations(List<Relation> out, Set<String> allowed, String src,
        ...
        if (dst == null || src.equals(dst) || !allowed.contains(dst))
            return;
    ...
    private static Set<String> referencedTypeNames(Type type) {
        ...
        while (tokens.hasMoreTokens()) {
            ...
            if (primitives.contains(token) || keywords.contains(token))
                continue;
